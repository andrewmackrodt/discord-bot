...
export type DecoratorOptions = Pick<
    CommandOptions,
    'emoji' | 'title' | 'description' | 'separator' | 'lastArgIsText' | 'args'
>
...
type CommandPropertyDescriptor = TypedPropertyDescriptor<
    (message: Message<true>, ...args: string[]) => any
>
...
export function registerCommandsFromDecorators<T>(registry: CommandRegistry, instance: T) {
    ...
    for (const reg of Object.values(registered[cname])) {
        ...
        if (words.length > 1) {
            ...
            command.subcommands[word] = createCommand(
                word,
                reg.method,
                reg.options,
                command,
