...
export type Builder<T, U extends new (params: T) => InstanceType<U>> = {
    build: Record<string, never> extends ExtractRequired<T> ? () => InstanceType<U> : never
    toObject: () => Partial<T>
} & {
...
type IsContainsAllRequired<T, K extends keyof T> =
    Pick<ExtractRequired<T>, K> extends ExtractRequired<T> ? true : false
