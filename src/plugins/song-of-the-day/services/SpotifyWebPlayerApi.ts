...
import type { AccessTokenResponse, GetAlbumResponse, QueryArtistOverviewResponse, WebPlayerApiResponse } from 'spotify-web-player'
...
export class SpotifyWebPlayerApi {
    ...
    public async getArtist(artistId: string): Promise<QueryArtistOverviewResponse> {
        const accessToken = await this.getAccessToken()

        const { data: res } = await axios.get<WebPlayerApiResponse<'artistUnion', QueryArtistOverviewResponse>>('https://api-partner.spotify.com/pathfinder/v1/query', {
            params: {
    ...
    public async getAlbum(albumId: string): Promise<GetAlbumResponse> {
        const accessToken = await this.getAccessToken()

        const { data: res } = await axios.get<WebPlayerApiResponse<'albumUnion', GetAlbumResponse>>('https://api-partner.spotify.com/pathfinder/v1/query', {
            params: {
    ...
    protected async getAccessToken(): Promise<string> {
        if (this.accessTokenResponse) {
            const expiresIn = this.accessTokenResponse.accessTokenExpirationTimestampMs - new Date().getTime()
        ...
        this.accessTokenResponse = await axios.get<AccessTokenResponse>('https://open.spotify.com/get_access_token', {
            params: {
                reason: 'transport',
