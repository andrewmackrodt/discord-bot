...
export class SongOfTheDayNotificationService {
    static readonly NOTIFICATION_PICK_HOUR = 10
    static readonly NOTIFICATION_OPEN_HOUR = 16
    static readonly NOTIFICATION_STOP_HOUR = 18

    constructor(private readonly repository: SongOfTheDayRepository) {}

    async sendNotifications(guild: Discord.Guild, now = new Date()): Promise<void> {
        ...
        if (channel?.type !== ChannelType.GuildText) {
            console.error(
                `songOfTheDay: server ${guild.id} has bad notifications channel configuration`,
            )
            return
        }
        ...
        if (
            SongOfTheDayNotificationService.NOTIFICATION_PICK_HOUR <= nowHour &&
            nowHour < SongOfTheDayNotificationService.NOTIFICATION_OPEN_HOUR
        ) {
            if (isEventTypeProcessable(NotificationEventType.PICK)) {
                ...
                await channel.send(
                    `**<@${user.id}> you have been chosen to select the song of the day** :musical_note:`,
                )
        ...
        // and it is open to anyone on the server to add one
            ...
            SongOfTheDayNotificationService.NOTIFICATION_OPEN_HOUR <= nowHour &&
            nowHour < SongOfTheDayNotificationService.NOTIFICATION_STOP_HOUR
        ) {
