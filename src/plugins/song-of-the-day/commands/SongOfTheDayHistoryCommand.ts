...
export default class SongOfTheDayHistoryCommand extends AbstractSongOfTheDayHistoryCommand {
    ...
    protected get nextInteractionCustomId(): string {
        return Interactions.HistoryNext
    }

    protected get prevInteractionCustomId(): string {
        return Interactions.HistoryPrev
    }
    ...
    async history(message: Message<true>, userId?: string): Promise<Message> {
                    ...
                    fields: rows
                        .map((row, i) => {
                            ...
                            if (row.playcount && row.playcount_updated_at && row.release_date) {
                                ...
                                const daysBetweenRelease = getDaysBetween(
                                    playCountUpdatedDate,
                                    row.release_date.toString(),
                                )
                                const ppd = row.playcount / daysBetweenRelease
                                const daysBetweenAdded = getDaysBetween(
                                    playCountUpdatedDate,
                                    row.date,
                                )
