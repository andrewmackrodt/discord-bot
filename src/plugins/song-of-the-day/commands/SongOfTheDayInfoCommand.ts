...
export default class SongOfTheDayInfoCommand {
    ...
    public async info(message: Message<true>, url: string): Promise<Message> {
        ...
        try {
            ...
            const playCount = album.tracks.items.find(t => t.track.uri.endsWith(`:${trackId}`))?.track.playcount
            if ( ! playCount) {
            ...
            return message.reply(`:musical_note:  **${artist} - ${track.name}** has ${playCountFmt} plays`)
        } catch (e) {
            console.error(e)
