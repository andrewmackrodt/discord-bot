...
export default class SongOfTheDayAddCommand {
    ...
    async add(message: Message<true>, url: string): Promise<Message> {
        ...
        try {
            ...
            await this.addSongOfTheDay(sdk, trackId, user, message)

            // create entry in playlist
            await sdk.addTracksToPlaylist(playlistId, [
                `spotify:track:${trackId}`,
            ])

            return message.channel.send(
                success(
                    `song of the day added to playlist <https://open.spotify.com/playlist/${playlistId}>`,
                ),
            )
        } catch (e) {
            console.error(e)
    ...
    protected async addSongOfTheDay(
        ...
        try {
            ...
            const count = album.tracks.items.find((t) => t.track.uri.endsWith(`:${trackId}`))?.track
                .playcount
        ...
        return this.repository.addSongOfTheDay(message.guildId, user, track, message.id, playcount)
    }
}
